...
class MydemoCharm(CharmBase):
    ...
    def _on_config_changed(self, event):
        ...
        # redirect = self.config['redirect-map']
    ...
    def _pull_site_action(self, event):
        """Handler for the pull_site_action"""
        event.log('This is an event log message')
        event.log('Calling site')
        self._fetch_site()
        event.log('Site pulled')
        event.set_results({
            "result": "Site pulled",
            "key": "some value"
        })
